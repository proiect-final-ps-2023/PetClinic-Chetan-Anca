...
public class DoctorController {
    ...
    @PutMapping("/rating")
    public ResponseEntity updateRating(@RequestParam long id, @RequestParam int rate) {
        Doctor doctorReturned = doctorService.updateRating(id, rate);
        DoctorDto doctorDtoReturned = modelMapper.map(doctorReturned, DoctorDto.class);
        return ResponseEntity.status(HttpStatus.OK).body(doctorDtoReturned);
    ...
    @GetMapping("/xml")
    public void saveDoctorToXML(@RequestParam long id) {
        doctorService.saveDoctorToXML(id);
    }
}
