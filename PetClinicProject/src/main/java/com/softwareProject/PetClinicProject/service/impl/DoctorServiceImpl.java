...
public class DoctorServiceImpl implements DoctorService {
    ...
    @Override
    public Doctor updateRating(long id, int rate) throws DoctorNotFoundException {
        Optional<Doctor> doctorToUpdate = doctorRepository.findById(id);
        if (!doctorToUpdate.isPresent()) {
            throw new DoctorNotFoundException("Doctor with id " + id + " not found");
        }
        doctorToUpdate.get().setNumberOfRatings(doctorToUpdate.get().getNumberOfRatings() + 1);
        doctorToUpdate.get().setStarsSum(doctorToUpdate.get().getStarsSum() + rate);
        doctorToUpdate.get().setRating(doctorToUpdate.get().getStarsSum() / doctorToUpdate.get().getNumberOfRatings());
        return doctorRepository.save(doctorToUpdate.get());
    }
    ...
    @Override
    public void saveDoctorToXML(long id) throws DoctorNotFoundException {
        Optional<Doctor> doctor = doctorRepository.findById(id);
        if (!doctor.isPresent()) {
        ...
        try {
            ...
            marshaller.marshal(doctor.get(), new File("Doctor" + id + ".xml"));
